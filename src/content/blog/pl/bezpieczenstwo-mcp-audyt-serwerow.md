...
Ale MCP dokłada drugą powierzchnię: opisy narzędzi i wyniki ich wywołań mogą trafić do kontekstu modelu. Dokładny moment zależy od klienta - część ładuje definicje od razu, część wyszukuje je leniwie - lecz treść użytego narzędzia staje się wejściem dla modelu. Model nie oddziela danych od poleceń z gwarancją porównywalną do parametryzacji zapytania SQL. Serwer MCP może więc zaszkodzić na dwa niezależne sposoby: jako kod, który robi coś złego na twojej maszynie, i jako treść, która skłania agenta do zrobienia czegoś złego twoimi rękami.
...
4. **Transport i uwierzytelnienie.** Serwer lokalny przez stdio ogranicza kanał MCP do procesu uruchomionego przez klienta, ale ten proces nadal może czytać dostępne pliki, zmienne środowiskowe i wykonywać połączenia sieciowe z uprawnieniami użytkownika. Serwer zdalny to inny model: twoje zapytania i dane płyną do cudzej infrastruktury. Kto ją utrzymuje, jak uwierzytelnia klientów, co loguje, jak długo przechowuje dane i czy mówi o tym wprost? Brak jasnej odpowiedzi na te pytania sam w sobie jest odpowiedzią.
...
Po trzecie, lista dozwolonych narzędzi po stronie klienta. Serwer potrafi wystawiać dwadzieścia narzędzi, z których potrzebujesz trzech. Klient może zablokować wywołanie pozostałych przez reguły uprawnień. Sprawdź osobno, czy dany klient również usuwa ich definicje z kontekstu - sama blokada wykonania nie zawsze skraca prompt ani nie ukrywa opisu narzędzia.

Po czwarte, kontener albo inna izolacja dla serwerów, którym ufasz tylko warunkowo. Kontener bez montowania katalogów gospodarza, bez nadmiarowych uprawnień, bez trybu uprzywilejowanego i z ograniczonym wyjściem sieciowym istotnie zmniejsza zasięg szkód. Nie jest jednak magiczną granicą: błędne montowania, gniazdo Dockera, sekrety w środowisku i szeroka sieć potrafią oddać serwerowi prawie te same uprawnienia co host.
