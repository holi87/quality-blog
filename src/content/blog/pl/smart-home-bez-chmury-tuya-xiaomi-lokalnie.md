...
Naciskasz przycisk w aplikacji, a komenda do żarówki wiszącej trzy metry od ciebie leci przez serwer na innym kontynencie i wraca po sekundzie albo dwóch. Tak z pudełka działa większość tanich urządzeń Tuya i spora część ekosystemu Xiaomi. Ta sekunda opóźnienia to tylko najbardziej widoczny objaw głębszego problemu: dom, który przestaje działać, gdy padnie łącze albo serwer producenta. Pokażę, jak odzyskać sterowanie lokalne nad tym sprzętem - co da się uwolnić od chmury w całości, co tylko częściowo, a co najrozsądniej wymienić przy najbliższej okazji.
...
Urządzenia Tuya na Wi-Fi rozmawiają z chmurą szyfrowanym protokołem, ale większość wystawia też interfejs lokalny w twojej sieci - trzeba tylko znać klucz lokalny urządzenia. Na tym opierają się integracje społecznościowe typu Local Tuya, instalowane zwykle przez [HACS](/pl/blog/hacs-w-home-assistant-os/). Koncepcyjnie wygląda to tak: zakładasz konto na platformie deweloperskiej Tuya, wiążesz z nim konto swojej aplikacji mobilnej i odczytujesz klucze lokalne przypisane do urządzeń. Dokładna procedura zmienia się co kilka miesięcy razem z interfejsem platformy, więc zamiast przepisywać kroki, które zdążą się zestarzeć przed publikacją, odsyłam do aktualnej dokumentacji wybranej integracji - to pierwsza rzecz, którą jej autorzy aktualizują.
...
W praktyce operacja wygląda tak: kupujesz koordynator (niewielki adapter USB), stawiasz Zigbee2MQTT albo włączasz ZHA i parujesz urządzenia od nowa, jedno po drugim. Przełożenie dwudziestu czujników to jeden wieczór - każde urządzenie trzeba wprowadzić w tryb parowania przyciskiem albo szpilką, po czym zgłasza się już bezpośrednio do Home Assistant. Automatyzacje reagują natychmiast, bo cała pętla zamyka się w twoich czterech ścianach, a sieć Zigbee dodatkowo zyskuje, bo każde zasilane z gniazdka urządzenie wzmacnia ją jako przekaźnik.

Jedna rzecz, na którą warto się przygotować: egzotyczne modele bywają obsłużone niekompletnie - zgłaszają się, ale część funkcji wymaga dodatkowej definicji. Bazy wspieranych urządzeń obu projektów są publiczne, więc model da się sprawdzić przed zakupem albo przed wieczorem migracji. Poza tym zastrzeżeniem ta ścieżka usuwa chmurę w stu procentach i dlatego traktuję ją jako domyślną odpowiedź na pytanie „jak uwolnić urządzenia Tuya i Xiaomi": jeśli sprzęt mówi po Zigbee, wszystkie pozostałe rozdziały tego tekstu są dla ciebie opcjonalne.

## Xiaomi: tokeny miio, czujniki BLE i bramki

Ekosystem Xiaomi jest bardziej różnorodny niż Tuya, więc i mapa uwalniania ma więcej pól. Urządzenia Wi-Fi - odkurzacze, oczyszczacze powietrza, lampy, nawilżacze - rozmawiają protokołem miio, który działa lokalnie, a Home Assistant obsługuje go od lat. Potrzebny jest token urządzenia, czyli klucz, którym aplikacja producenta uwierzytelnia się przy rozmowie lokalnej. Sposoby zdobywania tokenów zmieniają się wraz z wersjami aplikacji, więc ponownie: aktualne metody znajdziesz w dokumentacji integracji, nie w moim wpisie. Po skonfigurowaniu sterowanie odbywa się po sieci lokalnej, choć samo urządzenie - jeśli ma dostęp do internetu - nadal może meldować się chmurze; co z tym zrobić, opisuję w rozdziale o migracji.
...
Najtańsza w usunięciu jest chmura, której nigdy nie kupisz. Przed każdym zakupem zadaję sobie trzy pytania. Pierwsze: **czy istnieje wariant Zigbee albo Thread?** Urządzenie radiowe z otwartym standardem jest lokalne z natury i nie wie nawet, czym jest internet - to eliminuje cały problem u źródła, zanim powstanie. Wersja Wi-Fi tego samego produktu bywa kilkanaście złotych tańsza i te kilkanaście złotych to dokładnie cena, za którą odsprzedajesz kontrolę nad urządzeniem.
...
Chmura producenta to dług spłacany w czterech ratach: opóźnienia, telemetria, ryzyko wyłączenia usług i dom, który umiera razem z łączem. Mapa wyjścia jest krótsza, niż się wydaje. Sprzęt Zigbee uwalniasz w całości, przenosząc go pod własny koordynator z Zigbee2MQTT albo ZHA - to ścieżka domyślna i najpewniejsza. Tuya na Wi-Fi ratujesz mostem typu Local Tuya z kluczami lokalnymi, akceptując koszt utrzymania; Xiaomi na Wi-Fi obsługujesz tokenami miio, czujniki BLE czytasz pośrednikami Bluetooth, a bramki producenta czynisz zbędnymi. Flashowanie ESPHome i Tasmota zostaje jako opcja dla wtajemniczonych tam, gdzie układy na to pozwalają. Nowe zakupy wybierasz według zasady Zigbee/Thread przed Wi-Fi i sterowanie lokalne przed chmurą, a to, czego uwolnić się nie da, zamykasz w VLAN. Najuczciwszy test na koniec migracji: wyjmij kabel internetu z routera i policz, co przestało działać. Wynik tej próby mówi więcej o twoim smart home niż jakakolwiek lista funkcji.
