...
To nie jest pech, to statystyka. Karty SD nie są projektowane do ciągłego zapisu, a Home Assistant z włączoną bazą historii pisze na nośnik praktycznie bez przerwy. Typowa karta w takiej roli wytrzymuje od roku do trzech lat. Dyski SSD i pamięci eMMC (mam HA Yellow, więc znam temat z autopsji) żyją dłużej, ale też nie wiecznie. Pytanie nie brzmi „czy nośnik padnie", tylko „kiedy" - i czy będziesz wtedy przygotowany.
...
Standardowa kopia obejmuje cały katalog konfiguracji: automatyzacje, sceny, skrypty, panele, konta użytkowników, skonfigurowane integracje, zainstalowane dodatki razem z ich danymi oraz - opcjonalnie - bazę historii. Wszystko ląduje w jednym zaszyfrowanym archiwum.
...
- **Home Assistant Cloud.** Jeśli płacisz za subskrypcję (głównie dla zdalnego dostępu i asystentów głosowych), dostajesz też miejsce na kopię w chmurze. Limit rozmiaru wymusza wykluczenie bazy historii, ale konfiguracja to dosłownie jedno kliknięcie.
...
Osobna decyzja dotyczy bazy historii. System pozwala wykluczyć ją z kopii i to wykluczenie potrafi zmienić archiwum z 4 GB na 200 MB. Kompromis, który polecam: kopie codzienne bez bazy (szybkie, małe, tanie w przechowywaniu) plus raz w miesiącu pełna kopia z bazą. Tracisz wtedy najwyżej miesiąc wykresów, a konfigurację - nigdy. Jeśli historia jest dla ciebie krytyczna (na przykład liczysz koszty energii rok do roku), rozważ przeniesienie bazy do zewnętrznego silnika na NAS-ie i objęcie jej osobną kopią.

Trzeci element, o którym prawie nikt nie pamięta: pilnowanie, czy kopie w ogóle się wykonują. Cicha awaria mechanizmu kopii to klasyka - NAS zapełnił dysk, autoryzacja Google wygasła, hasło do udziału SMB zmienione przy okazji porządków. System dalej działa, więc nikt niczego nie zauważa, a licznik dni od ostatniej udanej kopii rośnie. Home Assistant wystawia informację o ostatniej kopii jako encję, a dodatek od Google Drive ma osobny sensor stanu - zbuduj na tym prostą automatyzację: jeśli ostatnia udana kopia jest starsza niż 48 godzin, wyślij powiadomienie na telefon. Dwie minuty pracy, a zamyka najpodlejszy scenariusz: awarię odkrytą dopiero wtedy, gdy kopia jest potrzebna.
...
| --- | --- | --- |
| Sieć Zigbee (klucz sieci, parowania) | Częściowo na koordynatorze USB | ZHA i Zigbee2MQTT zapisują kopię ustawień koordynatora w plikach konfiguracji - trafia ona do kopii HA, ale odtworzenie na koordynatorze innego modelu wymaga osobnej procedury migracji |
| Baza historii (jeśli wykluczona) | Plik bazy danych poza kopią | Comiesięczna pełna kopia albo zewnętrzna baza na NAS-ie |
...
Najwięcej nerwów kosztuje punkt pierwszy. Jeśli twój koordynator Zigbee padnie razem z resztą sprzętu, samo przywrócenie kopii HA nie wystarczy - nowy koordynator tego samego typu da się odtworzyć z zapisanych ustawień, ale zmiana na inny model oznacza zwykle ponowne parowanie urządzeń. Warto mieć w szufladzie zapasowy koordynator tego samego modelu. Kosztuje tyle co dwie wtyczki, a oszczędza weekend chodzenia po domu z drabiną.
...
W pracy testera powtarzam to zespołom przy każdym planie ciągłości działania i dokładnie ta sama zasada obowiązuje w domu. Procedura testu jest prosta i bezpieczna, bo nie dotyka działającego systemu:
