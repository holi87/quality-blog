...
Doświadczony tester widział już wiele podobnych problemów. Każda awaria, każdy bug, każdy „nikt nie wiedział, że to się stanie" zostawia wzorzec. Z czasem powstaje **biblioteka mentalna**, której nie da się przekazać podręcznikiem.
...
- **duplikaty** - wynikające z równoległego użycia lub retry,
- **błędy integracji** - kontrakt API zmienił się, ale klient nie został zaktualizowany,
- **problemy przy równoległym użyciu systemu** - race conditions, deadlocki w UI,
- **regresja w starych funkcjach** - nowa zmiana łamie coś, czego nikt nie pamięta.

Mniej doświadczony tester może sprawdzić ścieżkę szczęśliwą i pójść dalej. Doświadczony pyta: „a co jeśli te dane są starsze niż ta funkcja?" - i znajduje coś, czego nikt nie szukał.
...
- **request w przeglądarce** - czy w ogóle leci do serwera,
- **response z backendu** - kod HTTP, body,
...
To umiejętność, która **najbardziej oszczędza czas zespołu**. I którą najtrudniej wyceniają osoby spoza QA, bo nie widać jej w story poincie.
...
- **ryzyka release'owe** od **drobnych niedoskonałości**,
- **błędy blokujące** od **akceptowalnych ograniczeń** (known issues).
...
- **dodatkowy checkbox** - efekt domyślny dla starych rekordów?
...
Doświadczony QA wie, że **mała zmiana w UI może oznaczać dużą zmianę w logice**. I że formularz, który „przecież zawsze działał", może paść właśnie dlatego, że ktoś dodał jeden checkbox.
...
Różnica nie jest w długości. Różnica jest w **wartości informacji** dla osoby, która ten bug naprawi. Pierwsze zgłoszenie wymaga rozmowy, żeby cokolwiek z nim zrobić. Drugie zgłoszenie pozwala od razu zacząć pracę.
...
- Co było **wcześniej awaryjne** (bug history)?
...
- **Analizuj błędy produkcyjne** - każdy bug to lekcja. Pytaj „dlaczego to nie zostało wykryte wcześniej?"
- **Pytaj developerów o przyczyny defektów** - root cause, nie tylko poprawkę.
- **Wracaj do starych bugów** - szukaj wzorców co kwartał.
...
- **Czytaj logi i requesty** - nawet jeśli nie musisz dziś, kiedyś będziesz wiedział co znaczy 401 vs 403.
...
- buduje umiejętności w innych - przez mentoring, dokumentację, code review.
