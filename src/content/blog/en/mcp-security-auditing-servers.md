...
But MCP adds a second surface that classic plugins never had: everything the server returns lands directly in the model's context. Tool descriptions enter the context the moment you connect. Tool results enter it on every call. And the context is the model's entire reality - inside it, the model does not separate data from instructions as sharply as we would like. An MCP server can therefore hurt you in two independent ways: as code that does something bad on your machine, and as content that talks your agent into doing something bad with your hands.
...
Third, a client-side allowlist of tools. A server may expose twenty tools of which you need three. The MCP client - Claude Code included - lets you narrow the available tools to an explicit list. A shorter list means less attack surface and less context consumed, a double win.

Fourth, a container or other isolation for servers you trust only conditionally. A server running in a container without access to the host filesystem, with its network restricted to the addresses it genuinely needs, can be as malicious as it likes - the blast radius is defined by the container wall, not by the author's good will.
