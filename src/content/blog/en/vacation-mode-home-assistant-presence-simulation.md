...
Presence simulation has one goal: from the outside, the house should look inhabited. It cannot guarantee prevention of a burglary, so it is only an addition to physical security, an alarm, and a response plan. A perfectly regular schedule may look artificial, which is why I use time windows rather than identical hours every day. Only rooms visible from outside need to take part.
...
Simulation is theatre for the outside; inward, the house watches through opening, motion, and leak sensors, certified autonomous smoke alarms, and cameras. Home Assistant integration should supplement local smoke alarm behavior, not replace it. A critical phone notification still depends on operating-system settings and cannot guarantee bypassing every mute mode. Motion without a prior door opening deserves urgent verification but may also come from a pet or sensor error. Measure building temperature and refrigerator temperature with separate sensors in suitable locations; one room sensor cannot detect both failures reliably.

The second shift is a defined escalation plan, because the fallback of "I'll go downstairs and check" does not exist. At my place, an automatic shut-off follows a leak alert only after testing the sensors, valve, and consequences of isolation. A false shut-off is not always free - the supply may also serve heating, irrigation, or fire protection. No acknowledgement routes the alert to Julia and then to the trusted person with the keys.
...
The second element is power backup: a UPS on the server and router provides runtime determined by actual load, capacity, and battery condition, and an integration may expose status sensors. Measure runtime in a loaded test. The third element is unattended restart: verify that the server and Home Assistant return after power restoration and that the helper state returns as configured. Also make a fresh backup with tested recovery.
...
- **Water valve** - check the installation instructions and every dependent system before closing it by hand; do not isolate water required by heating or fire protection.
